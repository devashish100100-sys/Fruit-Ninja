...
namespace Assets.Scripts.Examples.Chat
{
    ...
    try
    {
        // Old plugin check removed – just proceed normally.
        // If you face certificate issues, consider enabling this handler:
        webRequest.certificateHandler = new AcceptAllCertificatesSignedWithASelfSignedCertificate();
    }
    catch (System.Exception ex)
            {
                Debug.LogWarning("Android certificate handler fallback failed: " + ex.Message);
            }
        #endif
        ...
        // New internal fallback certificate handler
        private class AcceptAllCertificatesSignedWithASelfSignedCertificate : CertificateHandler
        {
            protected override bool ValidateCertificate(byte[] certificateData)
            {
                // Accept all certificates (useful for testing or self-signed servers)
                return true;
            }
        }
